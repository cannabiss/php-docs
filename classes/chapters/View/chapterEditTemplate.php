...
$chapterInfo .= "<form action='chapters' method='post'>
    <div class='form-group'>
        ...
        <input type='hidden' name='chapter_id' value='" . $data['chapter']['id'] . "' class='form-control'/>
         <input type='hidden' name='date_update' value='" . date('Y-m-d') . "' class='form-control'/>
        <label>Наименование:</label>
        <input type='text' class='form-control' name='name' value='" . $data['chapter']['name'] . "'/>
    </div>
    <div class='form-group'>
        <label>Описание:</label>
       <textarea class='form-control' rows='3' name='description'>" . $data['chapter']['description'] . "</textarea>
    </div>
    <button type='submit' class='btn btn-info' style='margin-left: 94%'>Save</button>
</form>";

Components::addBlockText($chapterInfo, $data['title'], 'auto');